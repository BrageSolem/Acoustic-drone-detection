...
rms = [np.sqrt(np.mean(m**2)) for m in [mic1, mic2, mic3, mic4]]
print("RMS: mic1={:.0f} mic2={:.0f} mic3={:.0f} mic4={:.0f}".format(*rms))
print("Loudest channel: mic{}".format(np.argmax(rms)+1))
...
1. Test with greater spacing of the mics - In progress

"""
# 1 framing of the signals to 32 ms based on the pre-filter paper
duration = 4
fs_pcm = len(mic1)/duration # 23437.5
frame_ms = 32 # ms
samples_per_frame = int(fs_pcm * frame_ms / 1000) # samples per frame of 32 ms
# CLAUDE ###

def estimate_interblock_offset(mic_b, mic_a, fs, max_offset_samples=8):
    """
    Use a small max_offset_samples since inter-block offset should be
    only a few samples at most, not 32!
    Physical max delay for 10cm spacing = 0.1/343 * 23440 = ~7 samples
    So cap at 8 to avoid false peaks
    """
    from scipy.signal import correlate
    corr = correlate(mic_a.astype(float), mic_b.astype(float), mode='full')
    center = len(mic_b) - 1
    lags = np.arange(-(len(mic_b)-1), len(mic_b))
    
    # Only search within ±8 samples
    search = corr[center - max_offset_samples : center + max_offset_samples + 1]
    search_lags = lags[center - max_offset_samples : center + max_offset_samples + 1]
    
    offset_samples = search_lags[np.argmax(np.abs(search))]
    offset_seconds = offset_samples / fs
    print(f"Inter-block offset: {offset_samples} samples ({offset_seconds*1e6:.1f} µs)")
    return offset_seconds

interblock_bias = estimate_interblock_offset(mic1, mic3, fs_pcm)
...
"""
# Makes all of the freqs to contribute equaly, despite some of the frq not having much energy/utility
...
def phat_weighted(cps_mics, fs, n_samples_frame, freq_low = 200, freq_high = 6000, beta=0.5, eps = 1e-13, use_adaptive_beta = False):
    """
    The function is used to filter out the correlation values from the frequency bins present outside of the desired band frequency.
    ...
    if use_adaptive_beta:
        #signal_p = np.mean(mag) # change to find the value per frame, not the whole magnitude
        #noise_p = np.median(mag) + eps
        #snr_est = signal_p / noise_p

        # Takes the estimate of signal and noise power per frame not per whole magniutude
        signal_p_frame = np.mean(mag[n, :])
        noise_p_frame = np.median(mag[n,:])
        snr_est = signal_p_frame / noise_p_frame
    ...
    weighted_cps *= band # the bins outside of the band will be zeroed because .....
...
for cps in cps_arr:
    gcc_mag = phat_weighted(cps, fs_pcm, samples_per_frame, use_adaptive_beta= False)
    gcc_list.append(gcc_mag)
...
lag_center = K_bins//2
#print(lag_center)
corr_mags = [gcc for gcc in gcc_list]
...
#pair_dists = [0.15, 0.21, 0.15, 0.15, 0.21, 0.15 ] # 0.15  for 15cm
pairs = [(0,1), (2,3)]
pair_names = ["1-2","3-4"]


# based on the book "Microphone Arrays Signal Processing Techniques and Application" by Michael Brandstein, Darren Ward, CH.8.3.3
# vector p describing the 3D distance of the mics from the origo, which is the centre of stm32 
# mic1, mic2, mic3, mic4 are the columns of the array
p_vector = np.array([
...
#new_list = [expression for item in iterable if condition]
# baseline vector between mic i and mic j
cone_angles = []
aggregated_time_delay_list = []

for pair_indx in range(len(corr_mags)):

    i, j = pairs[pair_indx]
    baseline_vec = p_vector[:,i] - p_vector[:,j] # mi1 - mi2,
    baseline_norm = np.linalg.norm(baseline_vec) #since they are vectors had to use the eulicidean norm
        
    #distance_mics = pair_dists[pair_indx]
    t_delay_max = baseline_norm / speed_of_sound
    ...
    for n in range(N_frames):
        ...
        a_frame = np.abs(corr_mags[pair_indx][n, lag_min:lag_max])  # only physically valid lags, f.ex frame 0 - lags from min to max
    ...
    # simple confidence: peak height vs median energy in the segment
        confidence_score[n] = a_frame[peak_this_frame] / (np.mean(a_frame) + 1e-15) 
...
#print(time_delays)
    ...
    # also the estimate of theta in microhpone array 8.2.3
    aggregated_time_delay = np.median(time_delays[idx])
    #aggregated_time_delay_list.append(aggregated_time_delay)
     # CLAUDE ###

    cross_block = {(0,2), (0,3), (1,2), (1,3)}
    if (i,j) in cross_block:
        aggregated_time_delay -= interblock_bias
    # CLAUDE ###

    aggregated_time_delay_list.append(aggregated_time_delay)

    # microphone arrays eq.8.11
    # Continue with explenation!!
    """
    “For a single microphone pair and its TDOA estimate, 
    the locus of potential source locations in 3-space which satisfy (8.10)
    corresponds to one-half of a hyperboloid of two sheets.”

    "For sources with a large source-range to microphone-separation ratio, the
    hyperboloid may be well-approximated by a cone with a constant direction
    angle relative to the axis of symmetry. The corresponding estimated direction
    angle, Oi' for the microphone pair is given by: eq:8.11 "

    by using the eq:8.11 the far - field approximation is used to implement the estimate of the direction of the sound source relative to the baseline between a mic pair.
    Due the system being expectedt to detect drones on distances of few houndret meters, the far - field estimation will be used
    """
   
    cos_arg = np.clip((speed_of_sound * aggregated_time_delay) / baseline_norm, -1, 1) # must ensure the cos will not be bigger or smaller then 1 or -1 
    est_cone_angle_rad = np.arccos(cos_arg)
    est_cone_angle_degree = np.degrees(est_cone_angle_rad)
    # save the angles for each pair 
    cone_angles.append(est_cone_angle_rad)
    

    #baseline_unit_vec = baseline_vec / baseline_norm
    #rhs_value = (speed_of_sound * aggregated_time_delay) / baseline_norm
...
cone_angles = np.array(cone_angles)
measured_tdoas = np.array(aggregated_time_delay_list)

#### #### #### # #############
# Explain!!!

# has to define the azimuth search grid
...
# has to fnd the way minimizing the lest square error criterion, eq:8.12 and later eq:8.13
# ##### ##############################
# EXPLAIN THAT !!!!!
R_test = 1 # dummy radius, to put the signal 
error_surface = []
...
for phi in azimuth_grid_rad:
    ...
    for pair_indx, (i,j) in enumerate(pairs):
        # eq: 8.10
        predicted_tdoa = (np.linalg.norm(q - p_vector[:, i]) - np.linalg.norm(q - p_vector[:, j]))/speed_of_sound

        #eq: 8.12 
        error = (measured_tdoas[pair_indx] - predicted_tdoa)**2
...
estimated_azimuth_rad = azimuth_grid_rad[best_indx]
estimated_azimuth_deg = azimuth_grid_deg[best_indx]
##################
print("Est azimuth in deg: ", estimated_azimuth_deg)
...
for i, angle in enumerate(cone_angles):
    print(f"Pair {pair_names[i]}: cone angle = {np.degrees(angle):.1f}°")


plt.figure()
plt.plot(azimuth_grid_deg, error_surface)
plt.xlabel("Azimuth (degrees)")
plt.ylabel("Error")
plt.title("Error surface")
plt.axvline(x=-90, color='r', linestyle='--', label='Expected (-90°)')
plt.axvline(x=estimated_azimuth_deg, color='g', linestyle='--', label=f'Estimated ({estimated_azimuth_deg:.1f}°)')
plt.legend()
plt.grid()
plt.show()
